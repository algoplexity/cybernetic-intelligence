# -*- coding: utf-8 -*-
"""MScFE 690 G3614 - All2RulesOnly.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ntdo-35lQsSmEbs-G7XZc1DthLoPy_U5
...
cd /content/drive/MyDrive/Colab Notebooks/Capstone/gif
...
folder_path = '/content/drive/MyDrive/Colab Notebooks/Capstone/eca_images_2r'
...
unique_filenames = filenames_all

for filename in unique_filenames:

  print(f'{filename} to be loaded')
...
def generate_eca(init, rules, steps):
  ...
  for i in range(steps):
    ...
    rows = [generate_row(pattern[i], rule) for rule in rules]

    # Multiply all row results
    next_row = rows[0]
    for row in rows[1:]:
      next_row = np.multiply(next_row, row)
...
# Rule 1
pattern1_rule1 = generate_eca(init, [rule1], steps)
pattern1_rule1[pattern1_rule1 > 0] = 1
ax[0].imshow(pattern1_rule1, cmap='gray')
ax[0].set_title(f'Rule 1')

# Rule 2
pattern1_rule2 = generate_eca(init, [rule2], steps)
pattern1_rule2[pattern1_rule2 > 0] = 1
ax[1].imshow(pattern1_rule2, cmap='gray')
ax[1].set_title(f'Rule 2')

# Composed rules
pattern1_combined = generate_eca(init, [rule1, rule2], steps)
pattern1_combined[pattern1_combined > 0] = 1
ax[2].imshow(pattern1_combined, cmap='gray')
ax[2].set_title(f'Composed Rules')
...
# Reshape patterns
pattern1_rule1_reshaped = pattern1_rule1.reshape(-1, len(init))
pattern1_rule2_reshaped = pattern1_rule2.reshape(-1, len(init))
pattern1_combined_reshaped = pattern1_combined.reshape(-1, len(init))

# Plot patterns
fig, axs = plt.subplots(1, 3, figsize=(12,4))

axs[0].imshow(pattern1_rule1_reshaped, cmap='gray')
axs[0].set_title('Rule 1')

axs[1].imshow(pattern1_rule2_reshaped, cmap='gray')
axs[1].set_title('Rule 2')

axs[2].imshow(pattern1_combined_reshaped, cmap='gray')
...
for gen in range(steps):
  ...
  for i,pattern in enumerate(patterns):
    if np.sum(patterns[gen]) > 0:
      ax = fig.add_subplot(1,1,1)
      pattern_img = patterns[gen].reshape(-1, len(init))
      ax.imshow(pattern_img, cmap='gray')

  plt.savefig(f'frame{gen}.png')
  plt.close()

# Save animation
with imageio.get_writer('eca_evolution.gif', mode='I') as writer:
  for i in range(steps):
...
# Generate patterns
patterns = generate_eca(init, rules, steps)
...
for i in range(steps):
  ...
  pattern = patterns[i]
  pattern = pattern.reshape(-1, len(init))
...
imageio.mimsave('eca_animation.gif', frames, duration=0.1)
...
for i in range(steps):
  ...
  ax = fig.add_subplot(50, 1, i+1)

  ax.imshow(pattern, cmap='gray', alpha=0.5)
...
imageio.mimsave('eca.gif', frames, duration=0.1)
...
def compress_image(input_image):
  ...
  for gen in range(num_generations):
    ...
    print(f'RCS Gen {gen} : best_objective : {best_objective}')
...
def objective_rs(solution, target_image, pymils):
  ...
  # Decode rules
  rule1, rule2 = decode_rules(bits_list)
  print(f'objective_rs : rule1 is {rule1}')
  print(f'objective_rs : rule2 is {rule2}')

  if (rule2 is None ):
    print(f'objective_rs : image does not exist!')
    return 10000.
  else:
    try:
      image = eca_images[rule1][rule2]
    except KeyError:
      print(f'objective rs : KeyError caught rule1 is {rule1} rule2 is {rule2}')
      return 10000.
    except Exception as e:
      print(f'objective rs : Exception caught rule1 is {rule1} rule2 is {rule2}')
      print(type(e))
      print(e)
      return 10000.
    else:
      print(f'objective rs : Other Error caught rule1 is {rule1} rule2 is {rule2}')
      return 10000.
...
def match_image(input_image):
  ...
  rule1, rule2 = decode_rules(best_solutions[-1].values.numpy().tolist())
...
compressed_observed.shape

import sys
sys.setrecursionlimit(10000)
best_solutions , best_objectives = match_image(compressed_observed)
rule1, rule2 = decode_rules(best_solutions[-1].values.numpy().tolist())
print(rule1, rule2, best_objectives[-1])
